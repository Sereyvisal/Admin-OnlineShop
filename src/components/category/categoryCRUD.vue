<template>
  <v-dialog v-model="dialog" persistent width="1000" style="overflow-x: hidden">
    <v-card>
      <v-card-text class="pa-0 pt-4 pr-4 d-flex justify-end">
        <!-- <v-row>
          <v-col cols="12" class="pt-5 pr-6 d-flex justify-end">
            <v-icon @click="$emit('update:dialog', false)" color="red" size="25px">
              mdi-close-thick
            </v-icon>
          </v-col>
        </v-row> -->
      ...
      <v-card-title class="title font-weight-bold ml-2 pt-1">Category</v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12" lg="5" md="5" sm="6">
            <v-sheet elevation="4" style="overflow-y: scroll" height="500" class="pl-2 pr-0 py-2">
              <v-treeview
                :items="categoryList"
                item-key="_id"
                item-children="items"
              ...
              >
                <template v-slot:append="{ item }">
                  <v-icon @click.stop="handleDeleteDialog(item)" color="#F44336"> mdi-trash-can-outline </v-icon>
                </template>
              </v-treeview>
            </v-sheet>
          </v-col>
              ...
              <v-col cols="12">
                <v-form v-model="valid" ref="form">
                  ...
                  <span class="d-flex justify-end pt-5">
                    ...
                    <v-btn color="error" outlined @click="handleClearForm"> clear </v-btn>
                  </span>
                </v-form>
              </v-col>
            </v-row>
