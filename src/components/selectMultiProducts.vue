...
export default {
  props: { dialogSelectProduct: Boolean, selectedProducts: Array },
  data() {
    return {
      search: {
        keyword: "",
      ...
      listInventory(this.search)
        .then(res => {
          ...
          for (var p of res.datas) {
            var temp = {};
            temp._id = p._id;
            temp.image = p.product_info.image;
            temp.description = p.product_info.product_id.description;
            temp.selling_price = p.product_info.selling_price;
            temp.category = p.product_info.product_id.category.name;
            temp.name = p.product_info.product_id.name;
            temp.product_no = p.product_info.product_id.product_no;
            temp.attr = getProductAttribute(p.product_info.attr);
            temp.qty = 0;
            temp.stock_qty = p.qty;
